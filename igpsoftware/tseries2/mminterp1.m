function [yq,syq,nyq]=mminterp1(x,y,xq,xwindow)
%MMINTERP1  Moving Median interpolation
%   [YQ,SYQ,NYQ]=MMFILT(X,Y,XQ) computes the moving median YQ at data
%   points XQ from the input data series Y with abcissae X. SYQ is the 
%   robust estimation of the standard deviation computed using the median 
%   of absolute deviations (MAD), NYQ is an array with the number of
...
for k=1:nq
   ...
   yq(k,:)=nanmedian(y(idx,:),1);
   nyq(k)=sum(idx);
   syq(k,:)=1.4826*nanmedian(abs(y(idx,:)-repmat(yq(k,:),[nyq(k) 1])),1);
end
