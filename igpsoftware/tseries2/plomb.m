...
singleFlag = false;
%HMif any([signal.internal.sigcheckfloattype(x,'single','plomb','X')...
%HM    signal.internal.sigcheckfloattype(t,'single','plomb','T')]) 
%HM  x = single(x);
%HM  t = single(t);
%HM  singleFlag = true;
%HMend
...
if isnumeric(t)
  ...
  if numel(t) == 1
    ...
    if ~isempty(t)
      ...
      %HM validateattributes(t(~isnan(t)),{'numeric'},{'real','increasing','finite','>=',0},'PLOMB','T',2);
      validateattributes(t(~isnan(t)),{'numeric'},{'real','finite','>=',0},'PLOMB','T',2);
    end
