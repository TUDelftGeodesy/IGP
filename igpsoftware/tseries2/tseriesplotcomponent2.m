function hf=tseriesplotcomponent2(ts1,ts2,item,project,spacing)
  ...
  for i=1:nstations
    ...
    if any(ismember({ts1.station},station))
      ...
      if ~isempty(year1)
         ...
         offset=level(i)*spacing(icomponent)-nanmean(data1(:,icomponent))*1000+nanmean(data1(ia,icomponent)-data(ib,icomponent))*1000;         
      else
         offset=level(i)*spacing(icomponent)-nanmean(data(:,icomponent))*1000;
      end
      hcol=col(mod(i,7)+1,:);
