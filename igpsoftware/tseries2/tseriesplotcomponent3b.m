function hf=tseriesplotcomponent3b(ts1,ts2,ts3,item,stations,project,spacing)
%TSERIESPLOTCOMPONENT2 Plot three timescomponents for multiple stations.
%   TSERIESPLOTCOMPONENT(TSERIES1,TSERIES2,TSERIES3,ITEM) plots the timeseries components
...
if nargin < 3, error('too few arguments (specify at least stationames and item');end
if ~isstruct(ts1) && ~isstruct(ts2) && ~isstruct(ts3)
...
if isempty(stations)
  %stations=unique([{ ts1.station } { ts2.station } { ts3.station }],'stable'); 
  stations=unique([{ ts1.station } { ts2.station } { ts3.station }]) 
  stations(ismember(stations,'CMC'))=[];
end
...
for icomponent=1:3
  ...
  for i=1:nstations
    ...
    year1=[];
    data1=[];
    if any(ismember({ts3.station},station))
      tseries=ts3(ismember({ts3.station},station));
      ...
      h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color',col(2,:));
      %h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color','b');%col(1,:));
      %hcol=get(h,'Color');
      hold all; 
      y1=min(y1,year(1));
      y2=max(y2,year(end));
    ...
    if any(ismember({ts2.station},station))
      ...
      if ~isempty(year1)
         [~,ia,ib]=intersect(floor(dyear2date(year1)),floor(dyear2date(year)));
         %sf=nanmean(data1(ia(end-10:end),icomponent))*1000-nanmean(data1(ia,icomponent)-data(ib,icomponent))*1000;
         sf=nanmean(data(ib(end-10:end),icomponent))*1000;
         if isnan(sf), sf=nanmean(data(end-10:end,icomponent))*1000;, end
         offset=level(i)*spacing(icomponent)-sf;         
      else
         offset=level(i)*spacing(icomponent)-nanmean(data(end-10:end,icomponent))*1000;
         year1=year;
         data1=data;
      ...
      hcol=col(mod(i,7)+1,:);
      %h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color',brighten(hcol,.0));
      h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color',col(4,:));
      %h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color','r');%col(2,:));
      %hcol=get(h,'Color');
      hold all; 
      y1=min(y1,year(1));
      y2=max(y2,year(end));
    ...
    if any(ismember({ts1.station},station))
      ...
      hcol=col(mod(i,7)+1,:);
      %h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color',brighten(hcol,-.5));
      h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color',col(6,:));
      %h=plot(year,data(:,icomponent)*1000+offset,'LineWidth',1,'Color','k');%col(3,:));
      %hcol=get(h,'Color');
      hold all;
      y1=min(y1,year(1));
      y2=max(y2,year(end));
    end      
    line([y1 ;y2],[ level(i)*spacing(icomponent) ; level(i)*spacing(icomponent) ],'LineStyle',':','Color','k');
